...
def load_GO_annot_new(filename):
    # Load GO annotations
    onts = ['mf', 'bp', 'cc']
    prot2annot = {}
    prot2go = {}
    ...
    with open(filename, mode='r') as tsvfile:
        ...
        for row in reader:
            prot, prot_goterms = row[0], row[1:]
            prot2annot[prot] = {ont: [] for ont in onts}
            prot2go[prot] = {ont: [] for ont in onts}
            for i in range(3):
                goterm_prot = [goterm for goterm in prot_goterms[i].split(',') if goterm != '']
                prot2go[prot][onts[i]] = goterm_prot
                goterm_indices = [goterms[onts[i]].index(goterm) for goterm in prot_goterms[i].split(',') if goterm != '']
                prot2annot[prot][onts[i]] = np.zeros(len(goterms[onts[i]]))
                prot2annot[prot][onts[i]][goterm_indices] = 1.0
                counts[onts[i]][goterm_indices] += 1.0
    return prot2annot,prot2go, goterms, gonames, counts
